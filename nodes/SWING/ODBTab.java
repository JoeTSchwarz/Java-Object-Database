    ...
    ((JButton) map.get("fclose")).addActionListener(e -> {
      ...
      if (dbName != null && dbName.length() > 0) try {
        if (odbService.forcedClose(dbName)) {
          ...
          if (odbMgr.log) odbMgr.logging(dbName+" is forced to close by Superuser "+ServerController.suID);
      ...
      if (dbName != null && dbName.length() > 0) {
        ...
        if (list.size() > 0) try {
          if (odbService.forcedRollback(dbName)) {
            ...
            if (odbMgr.log) odbMgr.logging(dbName+" is forced to rollback by Superuser "+ServerController.suID);
        ...
        if (list.size() > 0) {
          ...
          if (key != null && key.length() > 0) try {
            if (odbService.forcedRollbackKey(dbName, key)) {
              ...
              if (odbMgr.log) odbMgr.logging(key.substring(1)+" of "+dbName+" is rollbacked by Superuser "+ServerController.suID);
        ...
        if (list.size() > 0) {
          ...
          if (key != null && key.length() > 0) try {
            if (odbService.forcedFreeKey(dbName, key)) {
              ...
              if (odbMgr.log) odbMgr.logging(key.substring(1)+" of "+dbName+" is freed by Superuser "+ServerController.suID);
      ...
      if (dbName != null && dbName.length() > 0) {
        ...
        if (list.size() > 0) try {
          if (odbService.forcedFreeKeys(dbName)) {
            ...
            if (odbMgr.log) odbMgr.logging("All keys of "+dbName+" are freed by Superuser "+ServerController.suID);
