  ...
  public void run() {
    ...
    while (parms.active) {
      try {
        ...
        if (dbName.charAt(0) == '+') {
          int le = dbName.indexOf("|");
          uID = dbName.substring(0, le);
          dbName = dbName.substring(le+1);
        }
        ios.preset(); // a MUST
       ...
       case 33: // allObjects(String dbName, String clsName)
        ...
        case 34: // selectAll(String dbName, String vName, String pat)
          ...
          String pat = new String(ois.readBytes(), parms.charset);
          kLst = odMgr.getKeys(uID, dbName);
          boolean ok = true;
          for (String k : kLst) {
            byte[] obj = odMgr.read(uID, dbName, k);
        ...
        case 36: // selectAll(vName, "GT", 123456) or selectAll(vName, "GT", 123.456)
          ...
          String[] val = new String(ois.readBytes(), parms.charset).split(""+(char)0x00);
          //
          for (String k : kLst) {
