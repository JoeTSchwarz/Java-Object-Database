...
public class ODBWorker extends Thread {
  ...
  */
  public ODBWorker(SocketChannel soc, ODBManager odbMgr) {
    this.odbMgr = odbMgr;
    this.soc = soc;
  }
  //
  public void run() {
    ...
    while (soc != null) {
      try {
        ...
        charset = odbMgr.getCharset(dbName);
        if (charset != null) {
        ...
        case 0: // ODBConnect
          ...
          if (key == null) {
            odbMgr.uIDList.add(uID);
            odbMgr.workers.add(this);
            ios.writeMsg(priv+"/"+uID+"/"+odbMgr.broadcaster);
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" is connected.");
            ios.writeBool(true);
            break;
          }
          // illegal user
        ...
        case 2: // disconnect ODBConnect
          ...
          odbMgr.workers.remove(this);
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uid+" is disconnected.");
          exit();
          return;
        case 3: // getKeys(dbName)
          ios.writeList(odbMgr.getKeys(uID, dbName));
          break;
        case 4: // getLocalKey
          ios.writeList(odbMgr.getLocalKeys(uID, dbName));
          break;
        case 5: // getClusterKey
          ios.writeList(odbMgr.getClusterKeys(uID, dbName, ois.readToken( )));
          break;
        case 6: // add(dbName, key, obj)
          key = ois.readToken( ); // key
          odbMgr.add(uID, dbName, key, ois.readBytes());
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" adds Object with key: "+key+" to "+dbName);
          if (notify.contains(dbName)) odbMgr.BC.broadcast(11, uID+"|"+dbName, Arrays.asList(key+" is added by "+uID));
          break;
        case 7: // unlock(dbName) all keys
          ios.writeBool(odbMgr.unlock(uID, dbName));
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" unlocked all keys of "+dbName);
          break;
        case 8: // update(dbName, key, obj)
          key = ois.readToken( ); // key
          if (odbMgr.update(uID, dbName, key, ois.readBytes())) {
            ios.writeBool(true);
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" updates Object with key: "+key+" of "+dbName);
            if (notify.contains(dbName)) odbMgr.BC.broadcast(11, uID+"|"+dbName, Arrays.asList(key+" is updated by "+uID));
          } else {
            ios.writeBool(false);
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" failed to update "+dbName+". Unknown or locked Key: "+key);
          }
          break;
        case 9: // delete(dbName, key)
          key = ois.readToken( ); // key
          if (odbMgr.delete(uID, dbName, key)) {
            ios.writeBool(true);
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" deletes Object with key: "+key+" of "+dbName);
            if (notify.contains(dbName)) odbMgr.BC.broadcast(11, uID+"|"+dbName, Arrays.asList(key+" is deleted by "+uID));
          } else {
            ios.writeBool(false);
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" failed to delete "+dbName+". Unknown or locked Key: "+key);
          }
          break;
        case 10: // read(dbName, key). Regardless of locked or unlocked
          key = ois.readToken( );
          ios.writeObj(odbMgr.read(uID, dbName, key));
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" reads Object with key: "+key+" of "+dbName);
          break;
        case 11: // isExisted(dbName, key)
          ios.writeBool(odbMgr.isExisted(uID, dbName, ois.readToken( )));
          break;
        case 12: // isLocked(dbName, key)
          key = ois.readToken( );
          ios.writeBool(odbMgr.isLocked(uID, dbName, key));
          break;
        case 13: // lock(dbName, key)
          key = ois.readToken( );
          ios.writeBool(odbMgr.lock(uID, dbName, key));
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" locks Key: "+key+" of "+dbName);
          break;
        case 14: // unlock(dbName, key)
          key = ois.readToken( );
          ios.writeBool(odbMgr.unlock(uID, dbName, key));
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" unlocks Key: "+key+" of "+dbName);
          break;
        case 15: // rollback(dbName) or rollback(dbName, key)
          if (odbMgr.isAutoCommit(uID)) {
            ios.writeErr(uID+" sets autoCommit: NO rollback.");
            break;
          }
          if (ois.remainderLength() > 0) {
            key = ois.readToken( ); // key
            if (odbMgr.isLocked(uID, dbName, key)) {
              ios.writeBool(odbMgr.restoreKey(uID, dbName, key, false));
              if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" rollbacks Key: "+key+" of "+dbName);
            } else ios.writeErr(key+" must be locked before rollback.");
          } else {
            ios.writeBool(odbMgr.restoreKeys(uID, dbName, false));
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" rollbacks all keys of "+dbName);
          }
          break;
        case 16: // ODBCluster - connect(dbName, charset) - connect only if ODB exists
          if ((new File(odbMgr.db_path+dbName)).exists()) odbMgr.connect(uID, dbName, ois.readToken());
          break;
        case 17: // ODBConnect - connect(dbName, charset)
          boolean existed = (new File(odbMgr.db_path+dbName)).exists();
          if (priv < 1 && !existed) {
            ios.writeErr(dbName+" does not exist and "+uid+" lacks of RW-Privilege.");
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" failed to connect "+dbName+". Reason: privilege:"+priv);
            break;
          }
          if (priv > 0 || existed) {
            String cs = ois.readToken();
            odbMgr.connect(uID, dbName, cs);
            odbMgr.bindAgent("+"+uID+"|"+dbName, cs);
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(dbName+" (charset:"+cs+") connected with "+uID);
          } else ios.writeErr(dbName+" does not exist. Or "+uid+" lacks of RW-Privilege.");
          break;
        case 18: // close(dbName)
          odbMgr.close(uID, dbName);
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(dbName+" closed by "+uID);
          break;
        case 19: // save(dbName)
          odbMgr.save(uID, dbName);
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(dbName+" saved by"+uID);
          break;
        case 20: // xDelete
          key = ois.readToken( ); // key
          if (odbMgr.lock(uID, dbName, key) || uID.equals(odbMgr.lockedBy(dbName, key))) {
            if (odbMgr.delete(uID, dbName, key)) {
              if (odbMgr.isAutoCommit(uID) || odbMgr.restoreKey(uID, dbName, key, true)) {
                ios.writeBool(true);
                if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" xDeletes key: "+key+" of "+dbName);
                if (notify.contains(dbName)) odbMgr.BC.broadcast(11, uID+"|"+dbName, Arrays.asList(key+" is deleted by "+uID));
              } else ios.writeErr("xDelete/commit "+dbName+" @"+key+" failed.");
              break;
            }
            ios.writeErr("xDelete "+dbName+" @"+key+" failed.");
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" failed to xDelete key: "+key+" of "+dbName);
          } else ios.writeErr("xDelete: Cannot lock Key "+key);
          odbMgr.unlock(uID, dbName, key);
          ios.writeBool(false);
          break;
        case 21: // xUpdate
          key = ois.readToken( ); // key
          if (odbMgr.lock(uID, dbName, key) || uID.equals(odbMgr.lockedBy(dbName, key))) {
             if (odbMgr.update(uID, dbName, key, ois.readBytes())) {
               if (odbMgr.isAutoCommit(uID) || odbMgr.restoreKey(uID, dbName, key, true)) {
                 ios.writeBool(true);
                 if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" xUpdates key: "+key+" of "+dbName);
                 if (notify.contains(dbName)) odbMgr.BC.broadcast(11, uID+"|"+dbName, Arrays.asList(key+" is updated by "+uID));
               } else ios.writeErr("xUpdate/commit "+dbName+" @"+key+" failed.");
               break;
             }
             ios.writeErr("xUpdate "+dbName+" @"+key+" failed.");
             if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" failed to xUpdate key: "+key+" of "+dbName);
          } else ios.writeErr("xUpdate: Cannot lock Key "+key);
          odbMgr.unlock(uID, dbName, key);
          ios.writeBool(false);
          break;
        case 22: // commit
          if (odbMgr.isAutoCommit(uID)) break;
          key = ois.readToken( );
          if (odbMgr.isLocked(uID, dbName, key)) {
            ios.writeBool(odbMgr.restoreKey(uID, dbName, key, true));
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" commits Key: "+key+" of "+dbName);
          } else {
        ...
        case 23: // commit all transaction
          ios.writeBool(odbMgr.restoreKeys(uID, dbName, true));
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" commits all keys of "+dbName);
          break;
        case 24: // lockedBy(dbName, key)
          String id = odbMgr.lockedBy(dbName, ois.readToken( ));
          if (id == null) ios.writeMsg("?");
          else ios.writeMsg(id);
          break;
        case 25:  // ODBCluster - closeAgents & disconnect
          odbMgr.removeAgent(ois.readToken());
          odbMgr.workers.remove(this);
          exit();
          return;
        case 26: // set autoCommit ODBConnect
        case 27: // reset autoCommit ODBConnect
          odbMgr.autoCommit(uID, cmd == 26);
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+" sets autoCommit("+(cmd == 26?"true":"false")+")");
          ios.writeBool(cmd == 26);
          break;
        case 28: // isKeyFree
          ios.writeBool(odbMgr.isKeyFree(uID, dbName, ois.readToken( )));
          break;
        case 29: // add(dbName, key, obj, node)
          //String[] keys = ois.readToken( ).split("+");
          String[] keys = ODBParser.split(ois.readToken(), "+");
          if (odbMgr.isExisted(uID, dbName, keys[0])) {
            ios.writeErr(keys[0]+" exists.");
            break;
          }
          ODBCluster odbc = odbMgr.getAgent(keys[1]);
          if (odbc == null) {
            ios.writeErr("Unknown "+keys[1]);
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging("Unknown node "+keys[1]+". Add failed.");
          } else {
            odbc.add("+"+uID+"|"+dbName, keys[0], ois.readBytes());
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging("Add Object to node "+keys[1]);
          }
          break;
        ...
        case 32: // allClassNames(String dbName)
          kLst = odbMgr.getKeys(uID, dbName);
          ArrayList<String> cLst = new ArrayList<>();
          for (String k : kLst) {
            oov.view(odbMgr.read(uID, dbName, k));
       ...
       case 33: // allObjects(String dbName, String clsName)
        ...
        case 34: // selectAll(String dbName, String vName, String pat)
          ...
          ios.writeObjList(aLst);
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+": select("+dbName+","+key+","+pat+")");
          break;
        case 35: // SQL(String dbName, String sql)
          ...
          */
          ...
          ios.writeObjList(aLst);
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+": sql("+dbName+","+key+")");
          break;
        case 36: // selectAll(vName, "GT", 123456) or selectAll(vName, "GT", 123.456)
          ...
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+": select("+dbName+","+key+" "+val[0]+" "+val[1]+")");
          ios.writeObjList(aLst);
          break;
        case 37: // getClassName(dbName, key)
          oov.view(odbMgr.read(uID, dbName, ois.readToken()));
          ios.writeMsg(oov.getClassName());
          break;
        case 38: // allObjects(dbName)
          aLst.clear();
          kLst = odbMgr.getKeys(uID, dbName);
          for (String k:kLst) aLst.add(odbMgr.read(uID, dbName, k));
          ios.writeObjList(aLst);
          break;
        case 39: // notify(dbName, enabled) add/delete/update notify
          ...
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging(uID+": notify("+(enabled?"true":"false")+")");
          break;
        case 40:
          odbMgr.BC.broadcast(12, odbMgr.webHostName, Arrays.asList(dbName));
          break;
        //----------------------------------SuperUser---------------------------
        case 97: // restoreKey(dbName, key, mode)
          key = ois.readToken( ); // key
          ios.writeBool(odbMgr.restoreKey(uID, dbName, key, (new String(ois.readBytes())).equals("true")));
          break;
        ...
        default:
          ...
          if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging("Unknown Request:"+cmd);
        }
        ...
        try {
          if (soc == null) {
            kLst = odbMgr.userDBList(uID);
            if(kLst != null && kLst.size() > 0) {
              for (String dbN:kLst) odbMgr.unlock(uID, dbN);
            }
            if (uID.charAt(0) != '+' && odbMgr.log) odbMgr.logging("ODBServer was down or shutdown by Superuser.");
  ...
  private String authenticate(String encrypt) throws Exception {
    ...
    if (odbMgr.log) odbMgr.logging("User "+uid+" is unknown.");
