...
public class ODBIOStream {
  ...
  */
  ...
  /**
  writeKey
  @param key Object (String, long, serialized Object)
  @exception IOException thrown by JAVA
  */
  public void writeKey(Object key) throws IOException {
    String t = key.getClass().getName();
    if (key instanceof String) {
      int le = ((String)key).length()+1;
      if ((pos+le) >= MAX) enlarge(le);
      buf[pos++] = (byte)(le>>8);
      buf[pos++] = (byte) le;
      // x00: tag for String Key
      System.arraycopy(((char)0x00+((String)key)).getBytes(), 0, buf, pos, le);
      pos += le;
    }
    // 0x02 for Long key
    else if (key.getClass().getName().equals("java.lang.Long")) {
      if ((pos+11) >= MAX) enlarge(32);
      // length 0x0009, Tag: 0x02
      System.arraycopy(new byte[] {
                         (byte)0x00,
                         (byte)0x09,
                         (byte)0x02, // Tag for long Key
                         (byte)(((long)key) >> 56),
                         (byte)(((long)key) >> 48),
                         (byte)(((long)key) >> 40),
                         (byte)(((long)key) >> 32),
    ...
    // 0x03 for BigInteger key
    else if (key.getClass().getName().equals("java.math.BigInteger")) {
      byte[] bi = ((BigInteger)key).toByteArray();
      if ((pos+bi.length) >= MAX) enlarge(bi.length);
      buf[pos++] = (byte)(bi.length >> 8);
      buf[pos++] = (byte)(bi.length & 0xFF);
      buf[pos++] = (byte)0x03; // Tag for BigInteger key
      System.arraycopy(bi, 0, buf, pos, bi.length);
      pos += bi.length;
    } 
    // x01 for Serialized Object as key
      ...
      if ((pos+le) >= MAX) enlarge(le);
      buf[pos++] = (byte)(le>>8);
      buf[pos++] = (byte) le; // total Length
      buf[pos++] = (byte)0x01;// Tag: 0x01 for POJO key
      System.arraycopy(out.toByteArray(), 0, buf, pos, --le);
      oos.close();      
      pos += le;
  ...
  */
  ...
  /**
  read the content from SocketChannel to buffer
  @param soc SocketChannel
  @exception IOException thrown by JAVA
  */
  public void read(SocketChannel soc) throws IOException {
    ...
    if (buf[0] == (byte)0x1F && buf[1] == (byte)0x8B &&  buf[2] == (byte)0x08) {
      ...
      pos = 0; byte bb[] = new byte[65536]; // 64KB
  ...
  */
  public ArrayList<Object> readKeys() throws Exception {
    ...
    for (int i, l, sum = 0, le; sum < len; sum += (2+le)) {
      ...
      i = p+2;
      // String as key
      if (buf[i] == (byte)0x00) list.add(new String(buf, ++i, le-1, charset));
      // serialized Object as key
      else if (buf[i] == (byte)0x01) { 
        ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(buf, ++i, le-1));
        list.add(ois.readObject());
        ois.close();
      } // long value as Key
      else if (buf[i] == (byte)0x02) list.add((((long)(buf[i++]&0xFF) << 56) |
                        ((long)(buf[++i]&0xFF) << 48) | ((long)(buf[++i]&0xFF) << 40) | 
                        ((long)(buf[++i]&0xFF) << 32) | ((long)(buf[++i]&0xFF) << 24) | 
                        ((long)(buf[++i]&0xFF) << 16) | ((long)(buf[++i]&0xFF) <<  8) | 
                        ((long) buf[++i]&0xFF)));
      // BigInteger as key
      else if (buf[i] == (byte)0x03) {
        byte[] val = new byte[le];
        System.arraycopy(buf, i+1, val, 0, le);
  ...
  /**
  readObjList. Object could be String (0x00), long value (0x02) or serializable Object (0x01)
  @return Object ArrayList
  @exception Exception thrown by JAVA
  */
  public ArrayList<Object> readObjList() throws Exception {
    ...
    for (int i, l, sum = 0, le; sum < len; sum += (2+le)) {
      le = ((buf[p]&0xFF) << 8)+(buf[p+1]&0xFF); // length
      ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(buf, p+2, le));
      list.add(ois.readObject());
      ois.close();
      p += (2+le);
    }
  ...
  */
  ...
  /**
  ifException
  @exception IOException if there was an exception
  */
  public void ifException() throws IOException {
    int l = ((buf[7]&0xFF) << 8)|(buf[8]&0xFF);
    if (l == 0) return;
    throw new IOException(new String(buf,17+(((buf[5]&0xFF)<<8)|(buf[6]&0xFF)), l, charset));
  }
  /**
  readMsg
