...
@author Joe T. Schwarz (c)
*/
public class ODBService implements Runnable {// extends Thread {
  /**
  ...
  */
  public long ping(String node) {
    try {
      ...
      if (ip.length > 1) {
        SocketChannel soc = SocketChannel.
            open(new InetSocketAddress(ip[0],
  ...
  */
  public boolean forcedClose(String dbName) throws Exception {
    parms.BC.broadcast(9, parms.webHostName+(char)0x01+dbName, parms.nodes);
    return odMgr.forcedClose(dbName);
  }
  /**
  forcedFreeKey(dbName, key) forces to free a locked keys on local node
  @param dbName String
  @param key String
  @return boolean true if key is freed
  @exception Exception thrown by JAVA
  */
  public boolean forcedFreeKey(String dbName, String key) throws Exception {
    parms.BC.broadcast(4, parms.webHostName+(char)0x01+key+" of "+dbName+" is forced to unlock.", parms.nodes);
    return odMgr.odbRestoreKey("*", dbName, key, odMgr.getDBCommit(dbName));
  }
  /**
  forcedRollbackKey(dbName, key) forces to rollback Object with the key on local node
  @param dbName String
  @param key String
  @return boolean true if key is rollbacked
  @exception Exception thrown by JAVA
  */
  public boolean forcedRollbackKey(String dbName, String key) throws Exception {
    parms.BC.broadcast(4, parms.webHostName+(char)0x01+key+" of "+dbName+" is forced to rollback.", parms.nodes);
    return odMgr.odbRestoreKey("*", dbName, key, false);
  }
  /**
  ...
  */
  public boolean forcedFreeKeys(String dbName) throws Exception {
    parms.BC.broadcast(4, parms.webHostName+(char)0x01+"All keys of "+dbName+" is forced to unlock.", parms.nodes);
    return odMgr.odbRestore("*", dbName, odMgr.getDBCommit(dbName));
  }
  /**
  ...
  */
  public boolean forcedRollback(String dbName) throws Exception {
    parms.BC.broadcast(4, parms.webHostName+(char)0x01+"All keys of "+dbName+" is forced to rollback.", parms.nodes);
    return odMgr.odbRestore("*", dbName, true);
  }
  /**
  ...
  */
  ...
  //
  private boolean sendODMgr(String node, int cmd, String dbName) throws Exception {
    String[] ip = node.split(":");
    ...
    soc.write(ByteBuffer.wrap(ios.toByteArray()));
    soc.read(buf); // wait for reply
    buf.flip();
    soc.shutdownInput();
    soc.shutdownOutput();
    soc.close();
    return buf.array()[0] == (byte) 0x01;
  }
  /**
