...
public class ODBManager implements ODBEventListening {
  ...
  */
  ...
  /**
  forcedClose() closes a connected db (only on this node)
  @param dbName String
  ...
  */
  ...
  /**
  odbRestoreKey() returns true if transaction is commited/rollbacked successfully and all keys are unlocked
  @param uID String, UserID
  @param dbName String
  @param key String
  ...
  @return boolean, true key is unlocked by key owner
  */
  public boolean odbRestoreKey(String uID, String dbName, String key, boolean mode) {
    if (autoCom.get(uID) != null) return false; // nothing to commit nor rollback
    ConcurrentHashMap<String, String> kMap = kOwner.get(dbName);
    if (kMap.size() > 0) {
      ODMS odms = odmsLst.get(dbName);
      if (uID.equals(kMap.get(key)) || uID.indexOf("*") >= 0 || odms.isKeyDeleted(key)) {
        kMap.remove(key); // unlock key
        kOwner.put(dbName, kMap);
        if (odms == null) return true;
        if (mode) return odms.commit(key);
        return odms.rollback(key);
      }
    }
    if (uID.charAt(0) != '+' && uID.charAt(0) != '*') {
      dbName = '+'+uID+'|'+dbName;
      if (mode) {
             for (ODBCluster odbc : cluster) if (odbc.commit(dbName, key)) return true;
      } else for (ODBCluster odbc : cluster) if (odbc.rollback(dbName, key)) return true;
    }
    return false;
  }
  /**
  odbRestore() returns true if transaction is commited/rollbacked successfully and all keys are unlocked
  @param uID String, UserID
  @param dbName String
  @param mode booelan, true: commit, false: rollback
  @return boolean, true key is unlocked by key owner
  */
  public boolean odbRestore(String uID, String dbName, boolean mode) {
    if (autoCom.get(uID) != null) return false; // nothing to commit nor rollback
    ConcurrentHashMap<String, String> kMap = kOwner.get(dbName);
    if (kMap.size() > 0) {
      ODMS odms = odmsLst.get(dbName);
      ...
      for (String key : kLst) if (uID.equals(kMap.get(key)) || uID.indexOf("*") >= 0 || odms.isKeyDeleted(key)) {
        kMap.remove(key); // unlock key
        lst.add(key);
      }
      if (lst.size() > 0) {
    ...
    if (uID.charAt(0) != '+' && uID.charAt(0) != '*') {
      dbName = '+'+uID+'|'+dbName;
      if (mode) for (ODBCluster odbc : cluster) odbc.commit(dbName);
      else for (ODBCluster odbc : cluster) odbc.rollback(dbName);
    }
    return true;
  }
  /**
  ...
  */
  public boolean removeAgent(String uID) {
    ...
    for (String dbName:list) {
      if (auto) odbRestore(uID, dbName, true);
      else odbRestore(uID, dbName, false);
      onActive(uID, dbName);
    }
    return true;
  }
