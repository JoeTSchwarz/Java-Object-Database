...
public class eDict extends Application implements ODBEventListening {
  ...
  public void odbEvent(ODBEvent event) {
    Platform.runLater(() -> {
      ...
      switch (type) {
        ...
        if (p > 0 && !dict.getUserID().equals(node.substring(0, p)) && dict.odb.equals(node.substring(p+1))) refresh(msg);
        return;
      }
    });
  }
  ...
  private void addWord( ) {
    eParms ep = eDialog.dialog("AddWord", lab, null, null, true);
    if (ep == null) return;
    txtArea.setText(ep.word+":\n"+ep.meaning);
    dict.add(ep.word, ep);
    dict.getKeys();
    if (dict.words != null && dict.words.size() > 0) dic.setAll(dict.words);
  }
  private void update( ) {
    ...
    if (key == null) return;
    eParms ep = dict.read(key);
    if (ep == null) {
      txtArea.setText(key+" is UNKNOWN or LOCKED.");
      return;
    }
    if (dict.isLocked(key) && !dict.unlock(key)) {
      txtArea.setText(key+" is LOCKED");
      return;
    }
    ep = eDialog.dialog("Update", lab, ep.word, ep.meaning, false);
    if (ep == null) return;
    if (dict.update(key, ep)) {
      dict.getKeys();
      dic.setAll(dict.words);
  ...
  private void searchIt() {
    ...
    if (word != null) {
      eParms ep = dict.search(word);
      if (ep != null) txtArea.setText(word+":\n"+ep.meaning);
  ...
  public void chkItem( ) {
    try {
      ...
      eParms ep = dict.read(word);
      if (ep == null) return;
      txtArea.setText(ep.meaning);
    } catch (Exception ex) { }
  }
  private void exit() {
    dict.save();
