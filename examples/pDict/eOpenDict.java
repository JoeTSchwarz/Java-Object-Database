...
public class eOpenDict {
  ...
  */
  ...
  public eParms search(String key) {
    try {
      for (String k:words) if (k.equals(key)) return (eParms)odbc.read(odb, key);
    } catch (Exception ex) { }
    return null;
  }
  ...
  public boolean update(String key, eParms ep) {
    try {
      return odbc.xUpdate(odb, key, ep);
    } catch (Exception ex) {
      ex.printStackTrace();
    }
    return false;
  ...
  public void add(String key, eParms ep) {
    try {
      odbc.add(odb, key, ep);
      done = true;
    } catch (Exception ex) {
  ...
  public eParms read(String key) {
    ...
    try {
      return ((eParms)odbc.read(odb, key));
    } catch (Exception ex) { }
    done = false;
    return null;
  }
  ...
  private String[] odbs = { "vpDict", "jpDict", "cpDict" };
}
